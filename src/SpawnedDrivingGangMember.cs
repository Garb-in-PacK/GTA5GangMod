...
namespace GTA.GangAndTurfMod
{
    public class SpawnedDrivingGangMember : UpdatedClass
    {
        ...
        public MemberAIOptions MemberAIOptions { get; set; }
            ...
            if (vehicleIAmDriving.IsAlive && watchedPed.IsAlive)
            {
                ...
                if (destination != Vector3.Zero)
                {
                    ...
                    if (vehicleIAmDriving.Position.DistanceTo2D(MindControl.CurrentPlayerCharacter.Position) >
                            SpawningOptions.MaxDistanceCarSpawnFromPlayer * 8)
                    {
                        DespawnProcedure();
                        return;
                    }
                ...
                else
                {
                    ...
                    if (vehicleIAmDriving.Position.DistanceTo2D(MindControl.CurrentPlayerCharacter.Position) >
                            SpawningOptions.MaxDistanceCarSpawnFromPlayer * 3)
                    {
            ...
            else
            {
                    ...
                    (!playerAsDest || !playerInVehicle))
                {
                    if (playerAsDest && deliveringCar) //zix - extra config options
                    {
                        ...
                        if (!vehicleIAmDriving.IsOnScreen && SpawningOptions.ForceSpawnCars &&
                            watchedPed.RelationshipGroup == GangManager.PlayerGang.relationGroup)
                        {
                ...
                else
                {
                    if (!watchedPed.IsPlayer)
                    {
                        if (playerAsDest && playerInVehicle)
                        {
                            ...
                            if (SpawningOptions.ForceSpawnCars &&
                                watchedPed.RelationshipGroup == GangManager.PlayerGang.relationGroup &&
                            ...
                            if (MindControl.CurrentPlayerCharacter.IsInFlyingVehicle)
                            {
                                ...
                                watchedPed.Task.DriveTo
                                    ...
                                    MemberAIOptions.DriverWithDestinationDrivingStyle);
                            }
                            else
                            {
                                if (isFriendlyToPlayer)
                                {
                                    Function.Call(Hash.TASK_VEHICLE_ESCORT, watchedPed, vehicleIAmDriving,
                                        ...
                                        MemberAIOptions.DriverWithDestinationDrivingStyle, 30, 0, 35);
                                }
                                else
                                {
                        ...
                        else
                        {
                            ...
                            watchedPed.Task.ClearAll();
                            watchedPed.Task.DriveTo(vehicleIAmDriving, destination, 15, targetSpeed, MemberAIOptions.DriverWithDestinationDrivingStyle);
                        }
                    }
                }
            }
        ...
        /// <summary>
        /// Just drive, with no destination in mind
        /// </summary>
        public void DoCruise()
        {
            watchedPed.Task.CruiseWithVehicle(vehicleIAmDriving, 20, MemberAIOptions.WanderingDriverDrivingStyle);
        }
        ...
        public void ClearAllRefs(bool makeDriverRoamPostClear = false)
        {
            ...
            if (makeDriverRoamPostClear && watchedPed.IsInVehicle())
            {
                watchedPed.Task.CruiseWithVehicle(watchedPed.CurrentVehicle, 15,
                    MemberAIOptions.WanderingDriverDrivingStyle);
            }
            watchedPed = null;
            myPassengers.Clear();
        }
        ...
        public SpawnedDrivingGangMember(Ped watchedPed, Vehicle vehicleIAmDriving, SpawningOptions spawningOptions, MemberAIOptions memberAIOptions, Vector3 destination, bool isFriendlyToPlayer, bool playerAsDest = false, bool deliveringCar = false)
        {
